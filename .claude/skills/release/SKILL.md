...
- Desktop releases: sanity-check `desktop-latest/latest.json` points at the new
  tag, and confirm the in-app updater offers the new version. Also confirm the
  version-free installers on `desktop-latest` (`shk-desktop-<target>.dmg` etc.,
  generated by `generate-desktop-stable-assets.sh`) were re-uploaded: their
  sizes must match the new `shk-desktop_*` assets and
  `desktop-latest/shk-desktop-latest.json` must carry the new `release_tag`.
  The website links to these stable names, so a stale upload is user-visible.
