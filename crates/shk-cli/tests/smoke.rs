...
fn ci_init_github_dry_run_prints_workflow() {
    ...
    assert!(
        stdout.contains(concat!("SHK_VERSION=v", env!("CARGO_PKG_VERSION"))),
        "{stdout}"
    );
    // The tag has to be read back through the variable. Inlining it leaves
    // SHK_VERSION unused, which fails shellcheck SC2034 and so actionlint.
    assert!(
        stdout.contains(r#"gh release download "$SHK_VERSION" -R "$REPO""#),
        "{stdout}"
    );
