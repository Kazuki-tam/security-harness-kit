...
Supported hook mode tools are `claude-code`, `codex`, `cursor`, `copilot`, `antigravity`, `grok`, and `windsurf` (Windsurf Cascade). Cascade ignores hook stdout, so `--hook-mode windsurf` signals a block via exit code 2 + a stderr message rather than a stdout decision payload. Grok Build `PreToolUse` blocks with `{"decision":"deny","reason":...}` and exit 2. `UserPromptSubmit` blocks with `{"decision":"block","reason":...}` (exit 2 also blocks). `PostToolUse` stdout is ignored, so post hooks only audit.
