...
`https://github.com/Kazuki-tam/security-harness-kit/releases/download/v0.3.2/shk-cli-installer.sh`.
...
```yaml
      - uses: actions/cache@v4
        ...
        with:
          path: ~/.cargo/bin/shk
          key: shk-${{ runner.os }}-v0.3.2

      - name: Install shk
        if: steps.shk-cache.outputs.cache-hit != 'true'
        shell: bash
        run: |
          curl --proto '=https' --tlsv1.2 -LsSf https://github.com/Kazuki-tam/security-harness-kit/releases/download/v0.3.2/shk-cli-installer.sh | sh
```
