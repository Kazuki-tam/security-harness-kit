...
After the package is accepted into the Windows Package Manager community repository, Windows users can install the CLI with:

```powershell
winget install Kazuki-tam.shk
```

Tagged CLI releases attach `shk-winget-manifests.zip`, which contains the generated WinGet manifests for maintainers to submit to [`microsoft/winget-pkgs`](https://github.com/microsoft/winget-pkgs). Until that initial submission is accepted, use the PowerShell installer or manual `.zip` install.
...
Tagged releases publish platform archives, per-archive checksums, installers, a Homebrew formula, WinGet manifests, and a CycloneDX SBOM:
...
```powershell
winget uninstall Kazuki-tam.shk
```
