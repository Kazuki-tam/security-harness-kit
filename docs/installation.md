...
To install a pinned release, replace `latest/download` with `download/<tag>` (e.g. `download/v0.2.0`). For long-term distribution, copy the generated `shk.json` into a Scoop bucket repository. The generated manifest includes `checkver` and `autoupdate` metadata for bucket-based updates.
...
To install a pinned release, replace `latest/download` with `download/<tag>` (e.g. `download/v0.2.0`). For long-term distribution, copy the generated `shk.rb` into a Homebrew tap repository.
