...
Each tagged CLI release also publishes an npm package. The names `shk` and `shk-cli` were already taken on the npm registry, so the package is published as [`security-harness-kit`](https://www.npmjs.com/package/security-harness-kit); the installed command is still `shk`:
...
The package version matches the release version. To pin a release in CI:

```bash
npm install -g security-harness-kit@0.3.16
```
