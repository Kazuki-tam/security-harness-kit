...
| `shk hooks install-ai` (Codex) | `.codex/config.toml` | Remove the block delimited by `# shk-managed-start` / `# shk-managed-end`, and `features.hooks = true` if no longer needed. If you used `--apply-sandbox`, also review the top-level `sandbox_mode` and `approval_policy` values. |
