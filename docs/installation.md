...
WinGet is not listed as an official install method until `shk` is accepted into the Windows Package Manager community repository. Tagged CLI releases attach `shk-winget-manifests.zip`, which contains generated manifests for maintainers to submit to [`microsoft/winget-pkgs`](https://github.com/microsoft/winget-pkgs).

Until the initial package submission is accepted, Windows users should use the PowerShell installer or manual `.zip` install.
